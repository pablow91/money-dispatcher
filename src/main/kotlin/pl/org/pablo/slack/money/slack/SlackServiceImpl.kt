...
class SlackServiceImpl(
    ...
    override fun getBalance(slackRequest: SlackRequest): String {
        ...
        return balance.joinToString { "${slackUserService.getUserName(it.name)} ${it.value}" }
    }
    ...
    override fun getPaymentHistory(slackRequest: SlackRequest): String {
        ...
        return (user.payed + user.received).asSequence()
                .sortedBy { it.date }
                .joinToString(separator = "\n") { "${slackUserService.getUserName(it.payer.name)} -> ${slackUserService.getUserName(it.receiver.name)} - ${it.description} - ${it.value} - ${it.date}" }
    }
